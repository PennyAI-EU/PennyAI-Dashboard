(function () {
  ...
  const titleMap = {
    home: ['Welcome back','Your current lesson plan and results, all in one place.'],
  ...
  const firstName = () => (liveData?.profile?.name || 'Learner').trim().split(/\s+/)[0] || 'Learner';
  ...
  function render() {
    ...
    document.getElementById('learnerPortal').innerHTML = `
      <aside class="sidebar proto-sidebar"><div class="brand"><span class="brand-mark">P</span><strong>PENNY AI</strong></div><nav class="proto-nav-list">${navItems.map(x=>link(x[0],x[1])).join('')}<a class="proto-nav" href="?view=progress">More</a></nav><div class="prototype-note"><b>Production visual preview</b><span>This layout is ready for phased connection. No learner data, calls, or live services are active here yet.</span></div></aside>
  ...
  // Visual-only production release: intentionally no database or call-service requests.
  render();
})();
